 ...
 fetch("https://cdn.jsdelivr.net/gh/uttamsingh829/uttamsingh.java/projects.json")
  ...
  .then((data) => {
    ...
    data.forEach((card) => {
      ...
      cardElement.classList.add("p-4", "w-full", "md:w-1/2", "overflow-hidden");

      // Add the card content
      cardElement.innerHTML = `
 <!-- card1 -->
               <div class="wow slideInFromRight bg-blue-100/70 w-full h-full flex flex-row p-2 rounded-lg">
                 <div class="aspect-square h-32 mr-3">
                   <img loading="lazy" class="aspect-square object-cover my-auto object-center w-full h-full rounded-md" src="${card.image}" alt="${card.title}">
                 </div>
